...
public class UserService {
    ...
    public User register(String username, String password, String phoneNumber) {
        ...
        // #创建用户相关信息
        UserAddition userAddition = new UserAddition(user.getUserId());
    ...
    public User login(String username, String password) {
        ...
        if (userAddition == null) {
            userAddition = new UserAddition(user.getUserId());
            userAddition = userAdditionRepository.save(userAddition);
        }

        // 判断是否处在登录出错冻结状态
        if (userAddition.getLoginForbidTime() != null) {
            Long delta = userAddition.getLoginForbidTime().getTime() - (new Date()).getTime();
            if (delta > 0) {
                delta /= 1000; // 转换为秒
                String time;
                // 时间差转换为中文
        ...
        if (bcryptPasswordEncoder().matches(password, user.getPasswordHash())) {
            // 允许登录
            if (userAddition.getLoginFailedCount() > 0) {
                // 清空错误登录次数
                userAddition.setLoginFailedCount(0);
                userRepository.save(user);
            }
            return user;
        }
        // 设置错误次数+1
        int failCount = userAddition.getLoginFailedCount() + 1;
        userAddition.setLoginFailedCount(failCount);
        // 根据错误次数，设置不同的时间
        String msg = "账号或密码错误";
        ...
        if (time > 0) {
            userAddition.setLoginForbidTime(new Date(time));
        }
    ...
    public void modifyNickname(User user, String nickname) {
        UserAddition userAddition = userAdditionRepository.findByUserId(user.getUserId());
        Date prevTime = userAddition.getNicknameModifyTime();
        ...
        userAddition.setNicknameModifyTime(currTime);
        userRepository.save(user);
        userAdditionRepository.save(userAddition);
        return;
    }
